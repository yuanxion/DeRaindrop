...
class Trainer(object):
    def __init__(self):
        ...
        self.betas = (0.5, 0.99)
        self.figure, (
            [self.ax_rain, self.ax_gt],
            [self.ax_gen, self.ax_loss],
        ...
        # sampler
        sampler_train = RandomSampler(self.trainset, replacement=True)
        self.trainloader = DataLoader(
            # self.trainset, batch_size=self.batchsize, shuffle=True, drop_last=True
            self.trainset,
        ...
        self.valloader = DataLoader(
            self.valset, batch_size=self.batchsize, drop_last=True
        )
        print('trainset:', len(self.trainset))
        print('valset:', len(self.valset))

    def get_map_loss(self, mask_d_gen, mask_d_gt, mask_g_gen):
        z = Variable(torch.zeros(mask_d_gt.shape)).cuda()
        loss_gen = self.mse_loss(mask_d_gen, mask_g_gen)
        loss_gt = self.mse_loss(mask_d_gt, z)
    ...
    def forward(self, batch, img, gt):
        ...
        grid_rain = torchvision.utils.make_grid(
            img,
            nrow=2,
            padding=0,
        )
        self.ax_rain.imshow(
            grid_rain.cpu().detach().permute(1, 2, 0), label='img_rain'
        )
        ...
        self.ax_gt.imshow(
            grid_gt.cpu().detach().permute(1, 2, 0), label='img_gt'
        )

        grid_gen = torchvision.utils.make_grid(
            img_gen,
            nrow=2,
            padding=0,
        )
        self.ax_gen.imshow(
            grid_gen.cpu().detach().permute(1, 2, 0), label='img_gen'
        )
    ...
    def train(self):
        ...
        for epoch in range(self.epoches):
            for i, (img, gt) in enumerate(self.trainloader):
                ...
                loss_g = torch.clamp(loss_g, 0, 2).item()
                ...
                loss_d = torch.clamp(loss_d, 0, 2).item()
